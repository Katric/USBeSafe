...
def create_overlay():
    ...
    print("[INFO] Overlay erstellt:", OVERLAY_IMAGE)


def start_vm_headless():
    print("[INFO] Starte Headless-VM...")

    # Falls alte Sockets existieren
    if os.path.exists(QMP_SOCKET):
        os.remove(QMP_SOCKET)
    if os.path.exists(VIRTIO_SOCKET):
        os.remove(VIRTIO_SOCKET)
    ...
    qemu_cmd = [
        ...
        # virtio Kommunikations-Port
        "-chardev", f"socket,id=virtiocomm,path={VIRTIO_SOCKET},server,nowait",
        "-device", "virtserialport,chardev=virtiocomm,name=securepass",

        # QMP für Host-Steuerung
        "-qmp", f"unix:{QMP_SOCKET},server,nowait"
    ]

    print("[INFO] QEMU Command:")
    print(" ".join(qemu_cmd))
    print()
...
def wait_for_virtio_message():
    print("[INFO] Warte auf Nachrichten vom VM-Daemon...")
...
def qmp_exec(command):
    """Kleine Hilfsfunktion, um QMP-Kommandos zu senden."""
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as s:
            s.connect(QMP_SOCKET)
            time.sleep(0.2)
            s.recv(4096)  # Initial QMP handshake lesen
            s.send((command + "\n").encode())
...
def stop_vm(vm_process):
    print("[INFO] Stoppe VM...")
    vm_process.terminate()
    try:
        vm_process.wait(timeout=5)
    except subprocess.TimeoutExpired:
        print("[WARN] VM reagiert nicht – force kill.")
        vm_process.kill()


def cleanup_overlay():
    if OVERLAY_IMAGE.exists():
        print("[INFO] Lösche Overlay:", OVERLAY_IMAGE)
        OVERLAY_IMAGE.unlink()
...
def main():
    ...
    vm = start_vm_headless()

    try:
        result = wait_for_virtio_message()

        if result == "ok":
            print("[INFO] VM meldet: Scan OK")
            print("[INFO] Warte auf 'copy_done'...")

            # Virtueller USB-Stick wird extern erstellt/eingehängt
            # (Nicht Teil dieses Scripts)

            result2 = wait_for_virtio_message()
            if result2 == "copy_done":
                print("[INFO] Copy abgeschlossen. Fahre VM herunter.")
                stop_vm(vm)
                cleanup_overlay()

        elif result == "fail":
            print("[INFO] VM meldet: Scan FAIL")
            stop_vm(vm)
            cleanup_overlay()

        else:
            print("[WARN] Unbekannte Nachricht:", result)
            stop_vm(vm)
            cleanup_overlay()
